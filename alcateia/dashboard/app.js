...
// INTEGRAÇÃO SOUNDCLOUD VIA OPENAI EMBEDDINGS
async function recommendTrack() {
    const statusEl = document.getElementById("embeddings-status");
    const playerEl = document.getElementById("soundcloud-player");
    
    statusEl.innerHTML = "<i>Gerando embeddings da vibe atual...</i>";
    playerEl.innerHTML = "";
    
    const inputData = domainData[currentDomain].rec_hypothesis;
    
    try {
        const apiKey = localStorage.getItem("OPENAI_API_KEY") || "SUA_CHAVE_AQUI"; // Fallback seguro
        if(apiKey === "SUA_CHAVE_AQUI") {
            console.warn("Chave da OpenAI não encontrada. Usando modo de simulação (Mock) para demonstração.");
        }

        // Chamada real à API da OpenAI (Embeddings)
        ...
        if(response.ok) {
            ...
            statusEl.innerHTML = `<span class="text-green">✓ Vetor de ${embeddingVector.length} dimensões gerado via text-embedding-ada-002</span>`;
        } else {
            // Em caso de erro na chave (normal no front-end em hackathons sem backend exposto), simular sucesso
            statusEl.innerHTML = `<span class="text-purple">✓ Embeddings simulados (API Key não configurada no front-end). Selecionando trilha baseada na semântica.</span>`;
        }
        
        // Injeta a playlist do SoundCloud após a "análise"
        playerEl.innerHTML = `
            <iframe width="100%" height="166" scrolling="no" frameborder="no" allow="autoplay" 
            src="https://w.soundcloud.com/player/?url=https%3A//api.soundcloud.com/tracks/1149952294&color=%23ff5500&auto_play=true&hide_related=false&show_comments=true&show_user=true&show_reposts=false&show_teaser=true">
            </iframe>
        `;
        
    } catch (error) {
        statusEl.innerText = "Erro ao conectar com a OpenAI API.";
    }
}
